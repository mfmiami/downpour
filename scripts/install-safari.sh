...
echo "Installing Downpour (Safari) $VERSION..."
...
if [[ -d "$DEST" ]]; then
  echo "Replacing existing Safari installation."
fi
install_app
...
display dialog "Downpour for Safari is installed.
...
If Downpour does not appear, quit Safari completely and reopen it." buttons {"OK"} default button "OK" with title "Safari Extension Installed" with icon note
APPLESCRIPT

echo "Safari install done."