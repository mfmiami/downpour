#!/usr/bin/env bash
# Copy extension source into the Safari Xcode wrapper and verify versions match.
set -euo pipefail

SRC="$(cd "$(dirname "$0")" && pwd)"
DEST="$(cd "$SRC/../VideoStreamDownloader-Safari/Video Stream Downloader/Shared (Extension)/Resources" && pwd)"
PBXPROJ="$SRC/../VideoStreamDownloader-Safari/Video Stream Downloader/Video Stream Downloader.xcodeproj/project.pbxproj"

FILES=(
  manifest.json
  background.js
  overlay.js
  content.js
  platforms.js
  popup.js
  popup.html
  instagram.js
  instagram-injected.js
  tiktok-extract.js
  tiktok-injected.js
  social-extract.js
  social-injected.js
)

echo "Syncing videodownload → Safari Resources"
for file in "${FILES[@]}"; do
  cp "$SRC/$file" "$DEST/$file"
  echo "  ✓ $file"
done

if [[ -d "$SRC/icons" ]]; then
  rsync -a --delete "$SRC/icons/" "$DEST/icons/"
  echo "  ✓ icons/"
fi

MANIFEST_VER="$(python3 -c "import json; print(json.load(open('$SRC/manifest.json'))['version'])")"
XCODE_VER="$(grep -m1 'MARKETING_VERSION' "$PBXPROJ" | sed 's/.*= *//;s/;//;s/^[[:space:]]*//')"

if [[ "$MANIFEST_VER" != "$XCODE_VER" ]]; then
  echo "ERROR: version mismatch — manifest.json=$MANIFEST_VER, Xcode MARKETING_VERSION=$XCODE_VER" >&2
  exit 1
fi

echo "Version check passed: $MANIFEST_VER"
echo "Done."