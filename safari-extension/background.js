...
function refererForJob(job) {
  if (job.eromeReferer) return String(job.eromeReferer).split("#")[0];
  if (job.watchUrl) return String(job.watchUrl).split("#")[0];
  return null;
}

function isYtDlpPageUrl(url) {
  if (typeof DownpourPlatforms !== "undefined" && DownpourPlatforms.isYtDlpPageUrl) {
    return DownpourPlatforms.isYtDlpPageUrl(url);
  }
  ...
  if (/\.(mp4|webm|m4v|mov|mkv|m3u8|mpd|ts|m4s)(\?|$)/i.test(url)) return false;
  try {
    const u = new URL(url);
...
async function fetchBytesWithProgress(url, job, signal) {
  ...
  if (useTab) {
    try {
      ...
      const bytesPromise = fetchViaTab(job.tabId, url, "bytes");
      ...
      const bytes = await Promise.race([
        ...
        new Promise((_, reject) => {
          const timer = setInterval(() => {
            ...
            clearInterval(timer);
            reject(new Error("Page download stalled — try another link or “This page”"));
          }, 2000);
          bytesPromise.finally(() => clearInterval(timer));
        })
      ]);
      return bytes;
    } catch (e) {
...
async function runStreamJob(job, options) {
  ...
  try {
    if (!rethrow) {
      try {
        if (await tryChromeStreamYtDlp(job, rethrow)) return;
      } catch (e) {
        if (wasCancelled(job, e)) {
...
async function tryChromeNativeFileDownload(job, rethrow) {
  if (!usesChromeDownloads() || !(await nativeHostReachable())) return false;
  if (job.imageDownload || !isDirectMediaFileUrl(job.url)) return false;
  try {
    ensureNotCancelled(job);
    assertFetchable(job.url);
    await runNativeUrlDownload(job, { referer: refererForJob(job) });
    return true;
  } catch (e) {
    if (wasCancelled(job, e)) throw e;
    if (rethrow) throw e;
...
async function tryChromeStreamYtDlp(job, rethrow) {
  if (!usesChromeDownloads() || !(await nativeHostReachable())) return false;
  if (!/\.m3u8(\?|$)/i.test(job.url || "")) return false;
  try {
    ensureNotCancelled(job);
...
async function runNativeUrlDownload(job, options) {
  ...
  try {
    ...
    const begin = await sendNative({
      ...
      url: job.url,
      filename: job.filename,
      referer: (options && options.referer) || refererForJob(job) || undefined
    });
    if (!begin || !begin.ok || !begin.token) {
...
async function runDirectJob(job, options) {
  ...
  if (!rethrow) {
    try {
      if (await tryChromeNativeFileDownload(job, rethrow)) {
        delete controllers[job.id];
        return;
      }
    } catch (e) {
